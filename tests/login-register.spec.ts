...
test('Register Test', async ({ page }) => {
  ...
  await page.locator('[id="customer\\.username"]').fill(randomSuffix);
  await page.locator('[id="customer\\.password"]').fill('Munna@2001');
  await page.locator('#repeatedPassword').fill('Munna@2001');
  await page.getByRole('button', { name: 'Register' }).click();
  await expect(page.getByRole('heading', { name: 'Welcome '+randomSuffix })).toBeVisible();
  await expect(page.getByText('Your account was created')).toBeVisible();
  await expect(page.getByRole('link', { name: 'Log Out' })).toBeVisible();
  await page.getByRole('link', { name: 'Log Out' }).click();
  await expect(page.getByRole('button', { name: 'Log In' })).toBeVisible();
  await page.getByRole('paragraph').filter({ hasText: 'Forgot login info?' }).click();
  await expect(page.getByRole('link', { name: 'Register' })).toBeVisible();
  await expect(page.locator('input[name="username"]')).toBeVisible();
  await page.locator('input[name="username"]').fill(randomSuffix);
  await page.locator('input[name="password"]').fill('Munna@2001');
